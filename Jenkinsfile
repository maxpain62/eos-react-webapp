    ...
    node (POD_LABEL) {
      ...
      stage ('build docker image and push to ecr repository') {
        container('buildkit') {
          ...
          --local dockerfile=.\
          --output type=image,name=134448505602.dkr.ecr.ap-south-1.amazonaws.com/dev/eos-react-webapp:1.18.0,push=true
          """
        }
      }
      stage ('package helm chart and push aws ecr repository') {
        container('aws-cli-helm') {
          sh """
            ...
            helm push eos-react-webapp-0.4.0.tgz oci://134448505602.dkr.ecr.ap-south-1.amazonaws.com/dev/helm/
